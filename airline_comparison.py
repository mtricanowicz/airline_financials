...
airline_financials["Operating Income"] = airline_financials["Operating Revenue"] - airline_financials["Operating Expenses"]
airline_financials["Operating Margin"] = round((airline_financials["Operating Income"] / airline_financials["Operating Revenue"]) * 100, 2)
airline_financials["Net Margin"] = round((airline_financials["Net Income"] / airline_financials["Operating Revenue"]) * 100, 2)
...
airline_financials["TRASM"] = airline_financials["Operating Revenue"] / airline_financials["ASM"]
airline_financials["PRASM"] = airline_financials["Passenger Revenue"] / airline_financials["ASM"]
airline_financials["CASM"] = airline_financials["Operating Expenses"] / airline_financials["ASM"]
        ...
        with st.container(border=True):
            ...
            with filter_col7:
                ...
                if st.session_state.metric_group_select=="All":
                    ...
                    st.session_state.selected_metrics = ["Operating Revenue", "Operating Income", "Net Income", "Operating Margin", "Net Margin"]
...
#####################################################################################
    ...
    for metric in selected_metrics:
        ...
        if metric in ["Operating Revenue", "Passenger Revenue", "Operating Expenses", "Operating Income", "Net Income", "Long-Term Debt", "Profit Sharing", "RPM", "ASM"]:
...
#####################################################################################
    ...
    with compare_tab1:
        for metric in selected_metrics:
            ...
            with compare_col1:
                ...
                if metric in ["Operating Revenue", "Passenger Revenue", "Operating Expenses", "Operating Income", "Net Income", "Long-Term Debt", "Profit Sharing"]:
    ...
    with compare_tab2:
        ...
        def format_value_based_on_metric(value, metric):
            if metric in ["Operating Revenue (millions)", "Passenger Revenue (millions)", "Operating Expenses (millions)", "Operating Income (millions)", "Net Income (millions)", "Long-Term Debt (millions)", "Profit Sharing (millions)"]:
...
#####################################################################################
    ...
    repurchase_authorization = {"AAL": None, "DAL": datetime(2025, 4, 29), "UAL": datetime(2024, 10, 15)}
    repurchase_resumption = {"AAL": None, "DAL": "2025Q2", "UAL": "2024Q4"}
    ...
    with col1:
        ...
        with col3:
            ...
            for airline in share_repurchases["Airline"].unique():
                ...
                if repurchase_authorization[airline] is not None:
                    st.markdown(f"{airline} repurchased **{total_shares_repurchase[airline]:.1f} million** shares at a total cost of **\\${(total_cost_repurchase[airline]/1000):.1f} billion**. The average share price of repurchase was **\\${total_average_share_cost[airline]:.2f}**. "
                                ...
                                f"{airline} authorized its first post-pandemic share repurchase program on **{repurchase_authorization[airline].strftime('%B %d, %Y')}** and resumed share repurchases during **{repurchase_resumption[airline]}**.<br>"
                                f"Based on the current share price and sales made during the pandemic, the repurchase campaign has netted {airline}:"
                                , unsafe_allow_html=True)
                else:
                    st.markdown(f"{airline} repurchased **{total_shares_repurchase[airline]:.1f} million** shares at a total cost of **\\${(total_cost_repurchase[airline]/1000):.1f} billion**. The average share price of repurchase was **\\${total_average_share_cost[airline]:.2f}**. "
                                f"To raise cash during the Covid-19 pandemic, {airline} offered and sold **{total_shares_sale[airline]:.1f} million** shares generating proceeds of **\\${(total_proceeds_sale[airline]/1000):.1f} billion**. The average share price of sale was **\\${total_average_share_sale[airline]:.2f}**. "
...
#####################################################################################
    with st.expander("Select an airline, year, and period to see the top insights.", expanded=True):
        ...
        st.info("Insights are extracted directly from the airlines' SEC filings. Summarizations are generated by AI and therefore may contain inaccuracies.", icon=":material/info:")
...
#####################################################################################
    ...
    if st.session_state.get_insights:
        ...
        if llm_airline==None or llm_period==None or llm_year==None:
            ...
            elif llm_airline not in ["AAL", "DAL", "UAL", "LUV"]:
                header_insights.header(f"Insights for {llm_airline} ({airline_names[llm_airline]}) | {llm_year}{llm_period}", divider='gray')
