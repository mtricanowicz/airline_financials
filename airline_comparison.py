...
st.set_page_config(
    ...
    menu_items={
        ...
        The LLM Insights tab allows the user to select one airline and year at a time and prompts ChatGPT to generate financial performance insights based on the airilne's SEC filings during the period. The accuracy of the content provided cannot be guaranteed.\n
...
#####################################################################################
    ...
    def get_sec_filings_summary(airline, year, period):
        prompt = f"""
        Summarize all SEC filings for {airline} from {year}{period}, including all 10-Q, 10-K, 8-K filings, annual reports, and other filings. 
        Provide the top insights for the year and period specified. Provide up to 10 insights. Otherwise, for a single quarter provide only 5 insights. Insights should be related to key developments in the following areas: financial, operational, commercial stratgy, labor, executive personnel, and employee profit sharing program. Do NOT include a topic if there is no relevant data or if there is nothing meaningful to report.
        Do NOT under any circumstances fabricate content. A fabrication is content not present in the SEC filings including but not limited to any mention of 'John Doe' or 'Jane Doe'.
        Be sure to highlight any major events and their impacts. 
        ...
        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an expert financial analyst summarizing SEC filings and presenting them for public consumption. Accuracy is paramount, but you should provide interesting and revelatory insights. Language and style should be a cross between a financial filing and business media reporting."},
