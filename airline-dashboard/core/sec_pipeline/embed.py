...
def retrieve_passages(
    ...
    """Retrieve ranked, unique passages across one or more queries.

    Results are fused across queries before duplicate suppression so repeated
    retrieval of the same evidence increases its relevance instead of being
    discarded as soon as it is first encountered.
    """
    collection = _client().get_collection(collection_name)
    ...
    # Aggregate all query hits before deduplication so repeated discovery of a
    # passage becomes a relevance signal rather than an accidental discard.
    candidates: list[dict] = []
    exact_matches: dict[tuple[str | None, str], dict] = {}

    def find_near_match(text: str) -> dict | None:
        shingles = _shingles(text)
        if not shingles:
            return None
        for candidate in candidates:
            candidate_metadata = candidate["metadata"]
            if candidate_metadata.get("source_id") != current_metadata.get("source_id"):
                continue
            candidate_index = candidate_metadata.get("chunk_index")
            current_index = current_metadata.get("chunk_index")
    ...
    for rank in range(max_len):
        for query_index, docs in enumerate(docs_per_query):
            ...
            distances = distances_per_query[query_index] if query_index < len(distances_per_query) else []
            ...
            current_metadata = dict(meta or {})
            key = (current_metadata.get("source_id"), _dedup_key(text))
            # Exact matches are source-scoped; near-duplicates are merged only
            # for adjacent chunks from the same filing.
            candidate = exact_matches.get(key) or find_near_match(text)
    ...
    for candidate in candidates:
        ...
        # Reciprocal-rank fusion rewards both high rank and cross-query support;
        # callers that omit weights retain the original equal-weight behavior.
        weights = query_weights or {}
        score = sum(
            weights.get(occurrence["query_index"], 1.0)
            / (60 + occurrence["query_rank"])
        ...
        metadata["query_indices"] = sorted({occurrence["query_index"] for occurrence in occurrences})
