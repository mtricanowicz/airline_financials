...
QUERY_WEIGHTS = {
    ...
    2: 0.90,  # capacity, traffic, fleet
    3: 0.75,  # labor
    4: 0.45,  # executive and board
    5: 0.95,  # route network
    6: 0.90,  # commercial strategy and loyalty
    7: 1.15,  # MD&A causes and offsets
    8: 0.95,  # unit economics
    9: 1.00,  # fuel
    10: 0.80,  # non-GAAP
    11: 0.65,  # risk and legal
...
SYSTEM_PROMPT = (
    "You are an evidence-first financial analyst writing SEC-based airline summaries "
    "for an informed general audience that includes investors, aviation professionals, "
    ...
    "preserve useful airline detail: operating metrics, unit economics, fleet and "
    "network decisions, liquidity, debt, labor agreements, and other filing-specific "
    "disclosures. Define specialized terms when a general reader may not know them, "
    "and explain why a metric matters only when that explanation is supported by the "
    "excerpts. Distinguish clearly between what the filing reports, what management "
...
def _retrieval_queries(
    airline: str, name: str, label: str, period_end: str
) -> list[str]:
    ...
    return [
        ...
        f"{base} unit revenue and airline operating metrics: RASM, PRASM, yield, CASM, load factor, RPM, and ASM.",
        f"{base} aircraft fuel prices, fuel expense, fuel hedging, consumption, and sensitivity to fuel costs.",
        f"{base} non-GAAP reconciliation, adjusted results, special items, and reconciliation from GAAP results.",
        f"{base} risk factors, legal proceedings, regulatory matters, litigation, and disclosed operational risks.",
        # 8-Ks are a separate recall channel for material events that may not
        # be explained in the periodic filing; routine 8-Ks can still be omitted.
        f"{base} material 8-K developments during or affecting {label}: major events, accidents, operational disruptions, executive or board changes, financing, aircraft or airport agreements, litigation, regulatory actions, and other period-specific disclosures.",
        f"{base} management forward guidance and outlook from earnings releases: next-quarter or full-year capacity, revenue, unit revenue, CASM, fuel, cash burn, liquidity, demand, and other quantitative ranges or targets.",
    ]
...
def _build_context(passages: list[tuple[str, dict]]) -> str:
    """Assemble relevant excerpts while avoiding forced topic coverage."""
    ...
    # Core channels may contribute multiple passages; secondary channels get a
    # single representative so broad retrieval does not force topic coverage.
    ...
    # Reserve a small amount of context for the dedicated guidance channel;
    # otherwise detailed historical results can crowd out earnings-release outlook.
    guidance = [
        passage
        for passage in ordered
        if GUIDANCE_QUERY_INDEX in passage[1].get("query_indices", [passage[1].get("query_index")])
    ]
...
- Treat fatalities, aircraft accidents, major operational disruptions, strikes, bankruptcies, regulatory actions, and material litigation as headline-level developments when supported. But do not include any mention of them if none were disclosed in the filings. For any relevant covered events state their disclosed human, operational, legal, or financial consequences directly; do not reduce a major event to a peripheral detail.
- Select a small set of non-overlapping business stories, not a complete inventory of filing topics. Before drafting, group related facts by the business question they answer: what changed, why it changed, what offset it, and what consequence followed. Each numbered item should tell one coherent story and include a figure with its comparison basis or a named specific; drop any candidate item that cannot meet that floor. A different metric, period aggregation, or section heading does not create a new insight when it describes the same underlying movement. Fold supporting metrics into the first relevant story and omit later restatements. Merge related metrics such as revenue, profit, and offsetting costs; RPM, ASM, and load factor; PRASM, yield, CASM, and fuel; liquidity and debt.
- Prioritize new or materially changed developments. Omit unchanged background, pre-existing investments or programs, and recurring descriptions unless they materially change the period's interpretation. Treat the period {label} as primary. Use accompanying year-to-date figures only when they change the interpretation of the period {label}, reveal a distinct cash or capital trend, or contain a material event not otherwise covered. Drop any YTD candidate that repeats a period {label} revenue, expense, fuel, labor, or operating trend. Keep each subject in its natural section.
- Write for an informed general reader: lead with plain-English meaning, retain useful airline detail, and define specialized acronyms at first use. Explain implications only when supported by the excerpts.
...
- Before drafting, merge and rank candidate developments by decision value, then assign each surviving item to the section that owns its underlying activity:
    - Financial covers revenue, expenses, profitability, margins, cash flow, balance sheet, liquidity, debt, dividends, and share repurchases. Keep cash-flow, debt repayment, liquidity, and capital allocation effects separate from net income and operating earnings. Debt repayments affect cash and liabilities; do not describe them as causes of net-income changes unless the filing explicitly states an accounting effect. Discuss the financial consequences of fleet or customer-facing infrastructure decisions under Financial only when they add a distinct capital, liquidity, debt, or earnings interpretation. Do not place operating or labor metrics in Financial merely because they affect financial results.
    - Operations covers capacity, fuel usage, traffic, load factor, fleet size and composition, utilization, reliability, and operating performance.
    - Labor covers workforce, wages, benefits, contracts, unions, staffing, furloughs, reductions in force, and labor relations.
    - Commercial Strategy covers loyalty, distribution, fares, product, pricing, customer economics, and major customer-facing airport or service projects such as gates, terminals, lounges, and related facilities.
    - Route Network covers routes, markets, airports, alliances, and fleet size, composition, aircraft orders, and fleet decisions when they affect network capability, capacity deployment, or customer access.
    - Corporate is an optional residual category for executive personnel, board changes, government affairs, litigation, investigations, regulatory actions, and compliance that do not belong in the core business sections. Include a Corporate item only when it materially affects the company's position, operations, strategy, financial exposure, risk, or outlook; routine litigation status, ordinary board appointments, and unchanged compliance disclosures do not qualify.
...
Using only the delimited filing excerpts above, produce the evidence-grounded summary for {name} ({airline}) for {label}."""
...
def summarize_period(
    ...
    period_end = f"{spec.period_end():%B %d, %Y}"
    queries = _retrieval_queries(airline, name, label, period_end)
    ...
    # Weighted rank fusion concentrates the context on explanatory and
    # period-defining evidence before the model performs final selection.
            ...
            messages=[
                {"role": "system", "content": SYSTEM_PROMPT},
                {
                    ...
                    "content": _user_prompt(
                        airline, name, label, period_end, context, compact=compact
                    ),
                },
            ],
        )
    ...
    if response.choices[0].finish_reason == "length":
        ...
        if response.choices[0].finish_reason == "length":
            raise RuntimeError(
                f"Summary exceeded the completion limit after compact retry: {airline} {label}"
            )
